...
def refresh_single_token(uid):
    guests = load_guests()
    user = next((g for g in guests if str(g.get("uid"))==str(uid)), None)
    if not user:
        return None
    try:
        res = requests.post("https://only-jwt.vercel.app/token", json={
            "uid": uid,
...
def update_token_file(uid,new_token):
    try:
        ...
        async with aiohttp.ClientSession() as session:
            async with session.post(url, data=edata, headers=headers, timeout=aiohttp.ClientTimeout(total=10)) as response:
                if response.status in [401,403]:
                    ...
                    new_token = refresh_single_token(token_value)
                    if new_token:
                        headers['Authorization'] = f"Bearer {new_token}"
                        update_token_file(token_value, new_token)
                        async with session.post(url, data=edata, headers=headers) as retry_response:
                            return retry_response.status
